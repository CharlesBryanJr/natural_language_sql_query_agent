...
def find_direct_matches(joins, table_name1, columns1, table_name2, columns2):
    matches = []
    for col in columns1:
        if col in columns2:
            matches.append({"table": table_name2, "column": col})
            joins.append({"table": table_name2, "column": col})
    return matches if matches else ["No direct matches"]
...
if __name__ == "__main__":
    ...
    for table_name1, table_data1 in data.items():
        #if counter >= max_loops:
            #break        
        joins = []
        columns1 = table_data1['columns']
        for table_name2, table_data2 in data.items():
            if table_name1 == table_name2:
                continue
            columns2 = table_data2['columns']
            result = find_direct_matches(joins, table_name1, columns1, table_name2, columns2)
            #print(f"Table1: {table_name1}")
            #print(f"Table2: {table_name2}")
            #print("Direct column name matches:")
            #print(result)
            #print(joins)
            #print('-'*30)
        print(f"Table1: {table_name1}")
